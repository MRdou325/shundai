...
$(function(){
    ...
    var alert_confirm = function (content,callback,width,tit){
        var width_str = '';
        if(width){
            width_str = 'width:'+width+'px';
        }
        $('#confirm_alert').remove();
        $('body').append('<div id="confirm_alert" style="display:none;'+width_str+'" >'+
        (tit?'<div class="tit">'+tit+'</div>':'')+
        (tit?'<a href="javascript:;" class="close_btn"></a>':'')+
        '<p>'+content+'</p>'+
