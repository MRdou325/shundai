...
$.fn.date_select=function(config){
    ...
    var _con = jQuery.extend({
        cur_date : $.merge([],now),
        today : $.merge([],now),
        clear_btn_display : true,
        min_date : null,//2013-6-6
        max_date : null,//2013-6-6
        day_limit: 0,//0表示无限制，1表示周一到周五，2表示周末
        data : '',
    ...
    return this.each( function() {
        ...
        var day = function(){
            ...
            for(i = from; i < to; i.setDate(i.getDate() + 1)){
                ...
                if(allow_day.indexOf(','+i.getDay()+',') === -1&&class_name.indexOf('disable') === -1){
                    class_name += ' disable';
                }
                var date_text = i.getFullYear()+'-'+(i.getMonth()+1)+'-'+i.getDate();
                if(con.data&&(con.data+',').indexOf(date_text+',')!== -1){
                    class_name += ' has_data';
                }
            ...
            var html = ''+
                '<div class="date_select" id="'+date_select_id+'"">'+
                    '<div class="date_select_yymm">'+
                        ...
                        '</div>'+
                    '</div>'+
                    (!con.clear_btn_display?'<a class="date_select_back_today" href="javascript:;">今天</a>':'')+
                    (con.clear_btn_display?'<a class="date_select_clear" href="javascript:;">清空</a>':'')+
