...
function get_date_price(type_id,x,y,z){
    ...
    $("#date_loading").html('<img src="http://www.cncn.com/images/loading.gif" style="display:block;width:28px;height:28px;margin:10px auto 10px auto;" title="加载中">');
...
function view_order(ths){//x当前日期,y当前欣欣价,z当前欣欣儿童价，k类别加月份，l当天
	...
	gt("starttime").value = arrx[0]+'-'+arrx[1]+'-'+arrx[2];
	//特价促销
	// if(gt("sale_price") && gt("discount").value>0){
	// 	y = y-gt("discount").value;
	// 	gt("sale_price").innerHTML = y;
	// }
	if(y){
		gt("bao_pric_span").innerHTML = y;
		gt("jd-current").innerHTML = y;
		gt("price_cncn").innerHTML = y;
		gt("adult_price").value = y;
	}
	if(gt("price_cncn_child")){
		if(z){
			gt("bao_pric_child_span").innerHTML = z;
			gt("price_cncn_child").innerHTML = z;
            ...
            if ($("input[name='child_num']").val()>0) {
                $("#child_message").html('该线路无儿童报价，具体价格请来电咨询');
            };
		}
	}
...
function fn_price(){
    ...
    for(var i = 0; i < y_m_arr.length; i++){
        ...
        for(j = from; j <= to; j.setDate(j.getDate() + 1)){
            ...
            if(j > temp_today){
                if(price){
                    lis += '<a class="price" href="javascript:;" onclick="view_order(this)" >'+price[0]+'<s>元</s></a>'
                }else{
                    lis += ''
                }
            }
