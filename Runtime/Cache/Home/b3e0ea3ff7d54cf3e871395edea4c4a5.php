...
<!-- /页头 -->
     ...
     <div class="box">
        <div class="col-md-3">
            <div id="side">
            <?php if(is_array($category)): $i = 0; $__LIST__ = $category;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$category): $mod = ($i % 2 );++$i;?><div<?php if($category["id"] == $_GET['cid']){ ?> class="on"<?php } ?>><a href="<?php echo U('Article/artlist',array('cid'=> $category["id"]));?>" title="<?php echo ($category["title"]); ?>"><?php echo ($category["title"]); ?></a></div><?php endforeach; endif; else: echo "" ;endif; ?>
            </div>
            <div id="side">
...
<div><?php echo C('WEB_SITE_TEL');?></div>
        ...
        <div class="tag_con" id="js_tag_con">                                                                        
            <div class="tu_wen">
                <ul>
                    <?php if(is_array($document)): $i = 0; $__LIST__ = $document;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$document): $mod = ($i % 2 );++$i;?><li>
                        <a href="<?php echo U('Article/detail',array('id'=> $document["id"]));?>" target="_blank" title="<?php echo ($document["title"]); ?>"><?php echo ($document["title"]); ?></a>
