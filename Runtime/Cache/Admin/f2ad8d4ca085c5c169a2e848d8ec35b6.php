		...
		<div class="form-item">
			<table id="orderinfo">
				<tr class="old">
					...
					<td colspan="5"><?php echo ($orders["goods_name"]); ?></td>
					<td class="title">出行时间</td>
					<td><?php echo ($orders["date"]); ?></td>
				</tr>
				<tr class="even">
					<td class="title">报名人数</td>
					<td><?php echo ($orders["num"]); ?>个大人，<?php echo ($orders["child"]); ?>个小孩</td>
					<td class="title">原价</td>
					<td><?php echo ($orders["price"]); ?></td>
					<td class="title">现价</td>
					<td><?php echo ($orders["discount_price"]); ?></td>
					<td class="title">儿童价</td>
					<td><?php echo ($orders["child_price"]); ?></td>
				</tr>
				<tr class="old">
				...
				<tr class="even">
					...
					<td colspan="7"><?php echo ($orders["content"]); ?></td>
				</tr>
			</table>
		</div>
	<form action="<?php echo U('status',array("id"=>$orders["id"]));?>" method="post" class="form-horizontal">
		<div class="form-item">
			<label class="item-label">当前状态<span class="check-tips"></span></label>
			<div class="controls">
				<select name="status">
					<?php if(is_array($statuslist)): $i = 0; $__LIST__ = $statuslist;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$list): $mod = ($i % 2 );++$i;?><option value="<?php echo ($list["id"]); ?>"<?php if($list['id'] == $orders['status']){ ?>selected<?php } ?>><?php echo ($list["name"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
				</select>
			</div>
		</div>
		<div class="form-item">
