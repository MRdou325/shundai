...
<body>
    ...
    <div class="cf">
        <a class="btn" href="<?php echo U('addstatus');?>">新 增</a>
        
        <!-- 高级搜索 -->
        <!-- <div class="search-form fr cf">
            <div class="sleft">
                <input type="text" name="name" class="search-input" value="<?php echo I('name');?>" placeholder="请输入幻灯片名称">
                <a class="sch-btn" href="javascript:;" id="search" url="/ke361/index.php?s=/Admin/Orders/orderStatus.html"><i class="btn-search"></i></a>
            </div>
        </div> -->
    </div>
                        ...
                        <table class="table table-striped table-bordered table-hover">
                            ...
                            <tbody>
                                <?php if(!empty($list)): if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$data): $mod = ($i % 2 );++$i;?><tr class="pointer even" title="">
                                            ...
                                            <td class="center"><a href="<?php echo U('Orders/editstatus',array('id'=>$data['id']));?>"><?php echo ($data['status']==1?'<font color="green">开启</font>':'<font color="red">关闭</font>'); ?></a></td>
                                            <td><a href="<?php echo U('Orders/addstatus',array('id'=>$data['id']));?>">编辑</a>
                                                            |
                                                <a onclick="return confirm('该分类下的商品将全部删除，确定删除')" href="<?php echo U('Orders/delstatus',array('id'=>$data['id']));?>" title="删除">删除</a>
                                            </td>
